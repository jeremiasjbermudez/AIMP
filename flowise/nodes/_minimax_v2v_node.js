// Video to Video: restyle or re-cast an existing clip.
//
// The same MiniMaxH3ReferenceToVideo node the Reference to Video tab already
// runs - it simply has more inputs than that tab uses. Alongside ref_images it
// accepts up to three ref_videos (IMAGE batches, 24fps, 2-15s), and feeding the
// source clip into one of those is what turns generation into transformation:
// the performance is carried by the video, while the reference images decide
// the look or the identity.
//
// Two things follow from it being the same node. No new model is loaded, and
// no second video stack is introduced - which matters, because mixing video
// models is exactly what this pipeline is not doing yet. And the output is a
// fresh render rather than an edit of pixels, so Face QA applies to the RESULT,
// not only to what went in.
//
// VHS_LoadVideoPath turns the source file into the frame batch the slot wants.
// force_rate 24 because the slot is specified at 24fps, and frame_load_cap
// bounds the 15s ceiling rather than trusting the clip to be short enough.
...
// VHS_LoadVideoPath wants a real path on disk, unlike LoadImage which reads a
// filename relative to input/. Clip paths are stored with mixed separators, so
...
if (!clip.source_clip_id) return { error: 'Pick the clip to transform.' };

// The clip being transformed. Its video is the performance; this row is the
// result, and stays linked to it so the pair can be compared later.
const srcRes = await axios.get(`${insforgeUrl}/api/database/records/minimax_clips`, {
  params: { id: `eq.${clip.source_clip_id}`, select: 'id,video_path,status,length,width,height' },
  headers: authHeaders
});
const source = (srcRes.data || [])[0];
if (!source) return { error: 'The clip being transformed no longer exists.' };
if (!source.video_path) return { error: 'That clip has no rendered video to transform.' };
...
const g = {
  ...
  '92': { class_type: 'SaveVideo', inputs: { video: ['130', 0], filename_prefix: movie.slug + '/_minimax_v2v/clip_' + clipId, format: 'auto', codec: 'auto' } }
};
...
// The source clip as a frame batch. VHS_LoadVideoPath reads an absolute path,
// so nothing needs staging into input/ the way LoadImage does.
//
// The slot is specified as 24fps and 2-15s. force_rate pins the first; the cap
// bounds the second - 360 frames is 15s, and a longer source is truncated
// rather than silently producing something the node will reject.
const sourceAbs = toAbs(source.video_path);
g['src_video'] = {
...
g['136'] = {
  ...
  inputs: {
    ...
    // Same flat dotted addressing as ref_images: COMFY_AUTOGROW_V3 slots are
    // NOT nested objects, and a nested one is read as a plain value with the
    // link silently never made.
    'ref_videos.ref_video_0': ['src_video', 0],
    // The source's own soundtrack, on the matching slot. Without it the model
    // invents new audio, so a restyle would quietly lose the performance's
    // voice and its lip sync along with it - which is precisely what "preserve
    // the performance" is supposed to mean. VHS_LoadVideoPath returns
    // (IMAGE, frame_count, audio, video_info), so audio is output 2.
    'ref_video_audios.ref_video_audio_0': ['src_video', 2],
    ...refImageInputs
  }
};
