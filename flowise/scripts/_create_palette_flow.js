...
(async () => {
  ...
  node.data.inputs.customFunctionJavascriptFunction = fs.readFileSync('C:/Flowise/_palette_namer_node.js', 'utf8');
  const made = await axios.post(BASE + '/api/v1/chatflows',
    { name: '31-Palette-Namer', flowData: JSON.stringify(fd), deployed: true, isPublic: false, type: 'AGENTFLOW' },
    { headers: H });
