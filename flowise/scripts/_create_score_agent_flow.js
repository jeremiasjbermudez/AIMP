...
(async () => {
  ...
  node.data.inputs.customFunctionJavascriptFunction = fs.readFileSync('C:/Flowise/_score_agent_node.js', 'utf8');
  const made = await axios.post(BASE + '/api/v1/chatflows',
    { name: '30-Score-Agent', flowData: JSON.stringify(fd), deployed: true, isPublic: false, type: 'AGENTFLOW' },
    { headers: H });
