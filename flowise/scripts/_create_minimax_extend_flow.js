// Creates (or updates) 16-MiniMax-Extend: continues an existing MiniMax H3 clip,
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_minimax_extend_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Extend MiniMax Clip', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '16-MiniMax-Extend', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
