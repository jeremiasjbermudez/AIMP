// Creates (or updates) 10-Qwen-Cleanup: the standalone version of the Qwen
// splat-cleanup that 6-GS-Cleaner runs inside the shot pipeline. Same graph,
// but driven by a qwen_cleanups row (any uploaded source + either an uploaded
// reference or a chosen scene's panorama) instead of a shot row.
//
//   node _create_qwen_cleanup_flow.js              -> creates
//   UPDATE_ID=<id> node _create_qwen_cleanup_flow.js -> updates in place
const fs = require('fs');
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_qwen_cleanup_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Run Qwen Cleanup', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '10-Qwen-Cleanup', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
