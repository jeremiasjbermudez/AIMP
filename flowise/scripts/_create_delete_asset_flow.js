...
(async () => {
  ...
  node.data.inputs.customFunctionJavascriptFunction = fs.readFileSync('C:/Flowise/_delete_asset_node.js', 'utf8');
  const made = await axios.post(BASE + '/api/v1/chatflows',
    { name: '29-Delete-Asset', flowData: JSON.stringify(fd), deployed: true, isPublic: false, type: 'AGENTFLOW' },
    { headers: H });
