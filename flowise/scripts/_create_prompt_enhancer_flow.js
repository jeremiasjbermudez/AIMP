// Creates (or updates) 11-MiniMax-Prompt-Enhancer: turns a draft prompt plus a
// motion request (and optionally the actual first frame) into a MiniMax H3
// image-to-video delta prompt, using qwen3.8's vision capability on the
// dedicated Ollama host.
//
//   node _create_prompt_enhancer_flow.js               -> creates
//   UPDATE_ID=<id> node _create_prompt_enhancer_flow.js -> updates in place
const fs = require('fs');
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_prompt_enhancer_node.js'), 'utf8');
const SYSTEM_PROMPT = fs.readFileSync(path.join(__dirname, '_minimax_i2v_prompt_instruction.md'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Enhance MiniMax Prompt', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '11-MiniMax-Prompt-Enhancer', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
