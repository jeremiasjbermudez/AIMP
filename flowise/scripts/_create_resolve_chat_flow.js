// Creates (or updates) 21-Resolve-Chat: continues an existing MiniMax H3 clip,
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_resolve_chat_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Resolve Chat', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '21-Resolve-Chat', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
