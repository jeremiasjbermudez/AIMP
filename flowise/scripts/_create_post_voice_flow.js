// Creates (or updates) 18-Post-Voice: continues an existing MiniMax H3 clip,
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_post_voice_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Post Voice', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '18-Post-Voice', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
