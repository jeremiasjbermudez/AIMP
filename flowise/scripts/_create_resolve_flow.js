// Creates (or updates) 20-Resolve-Deliver: continues an existing MiniMax H3 clip,
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_resolve_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Send to Resolve', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '20-Resolve-Deliver', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
