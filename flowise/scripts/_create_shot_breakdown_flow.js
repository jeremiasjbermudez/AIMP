...
(async () => {
  const code = fs.readFileSync('C:/Flowise/_shot_breakdown_node.js', 'utf8');
  let probe = code;
  ...
  try { new Function('require', 'process', 'return (async () => {' + probe + '})'); }
  ...
  if (id) await axios.put(BASE + '/api/v1/chatflows/' + id, { flowData }, { headers: H });
    ...
    const made = await axios.post(BASE + '/api/v1/chatflows',
      { name: '42-Shot-Breakdown', flowData, deployed: true, isPublic: false, type: 'AGENTFLOW' }, { headers: H });
    id = made.data.id;
  }
  ...
  console.log('42-Shot-Breakdown id:', id, '| matches:', live === code);
