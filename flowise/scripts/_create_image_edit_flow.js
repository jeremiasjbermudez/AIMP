// Creates (or updates) 26-Image-Edit: FLUX.2 Klein editing with one or more
// reference images, so scenes can be composed from the admin app rather than in
// ComfyUI directly.
//
//   node _create_image_edit_flow.js                -> creates
//   UPDATE_ID=<id> node _create_image_edit_flow.js -> updates
const fs = require('fs');
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_image_edit_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Klein Image Edit', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '26-Image-Edit', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
