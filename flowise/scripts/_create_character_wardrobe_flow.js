// Creates (or updates) 40-Character-Wardrobe: puts a costume on a character's reference sheet, so a
// multimodal qwen3.8 and gets back a 360 panorama prompt describing that place.
//
//   node _create_pano_prompt_flow.js                -> creates
//   UPDATE_ID=<id> node _create_pano_prompt_flow.js -> updates
const fs = require('fs');
...
// The dedicated box - qwen3.8 there reports the vision capability. Not the
// local Ollama, which shares its GPU with ComfyUI.
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_character_wardrobe_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Dress a Character', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '40-Character-Wardrobe', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
