// Creates (or updates) 17-MiniMax-Ref-To-Video: continues an existing MiniMax H3 clip,
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_minimax_ref_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Reference to Video', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '17-MiniMax-Ref-To-Video', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
