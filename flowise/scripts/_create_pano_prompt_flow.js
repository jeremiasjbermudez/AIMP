// Creates (or updates) 27-Pano-Prompt-From-Image: shows a project image to the
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_pano_prompt_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Describe a Panorama From an Image', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '27-Pano-Prompt-From-Image', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
