// Creates (or updates) 24-Face-QA: scores a rendered clip's face against a
// character's reference images. Measurement only - it never edits a clip.
//
//   node _create_face_qa_flow.js                -> creates
//   UPDATE_ID=<id> node _create_face_qa_flow.js -> updates
const fs = require('fs');
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_face_qa_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Score Face Drift', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '24-Face-QA', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
