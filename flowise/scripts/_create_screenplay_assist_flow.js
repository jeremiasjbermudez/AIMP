// Creates (or updates) 14-Screenplay-Assist: the model side of the Screenplay
// Builder. Three modes - breakdown, enhance, bible - and it writes nothing;
// every mode returns a proposal the UI shows for review.
//
//   node _create_screenplay_assist_flow.js               -> creates
//   UPDATE_ID=<id> node _create_screenplay_assist_flow.js -> updates
const fs = require('fs');
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_screenplay_assist_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Screenplay Assist', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '14-Screenplay-Assist', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
