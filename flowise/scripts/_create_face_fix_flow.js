...
(async () => {
  ...
  node.data.inputs.customFunctionJavascriptFunction = fs.readFileSync('C:/Flowise/_face_fix_node.js', 'utf8');
  const made = await axios.post(
    BASE + '/api/v1/chatflows',
    { name: '34-Face-Fix', flowData: JSON.stringify(fd), deployed: true, isPublic: false, type: 'AGENTFLOW' },
    { headers: H }
  );
