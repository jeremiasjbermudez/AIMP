// Creates (or updates) 46-MiniMax-Control-To-Video: a depth/pose/edge video drives a MiniMax H3 shot,
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_minimax_control_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Control to Video', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '46-MiniMax-Control-To-Video', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
