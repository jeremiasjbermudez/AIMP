// Creates (or updates) 25-Character-QA-Shots: turns one photo of a character
// into the reference angles face QA needs, using FLUX.2 Klein with the photo as
// a reference latent.
//
//   node _create_character_qa_shots_flow.js                -> creates
//   UPDATE_ID=<id> node _create_character_qa_shots_flow.js -> updates
const fs = require('fs');
...
const NODE_SOURCE = fs.readFileSync(path.join(__dirname, '_character_qa_shots_node.js'), 'utf8');
...
(async () => {
  ...
  const fnNode = {
    ...
    data: {
      id, label: 'Create QA Reference Shots', version: 1.1, name: 'customFunctionAgentflow', type: 'CustomFunction',
  ...
  try {
    if (process.env.UPDATE_ID) {
      ...
      const res = await axios.post(
        ...
        { name: '25-Character-QA-Shots', type: 'AGENTFLOW', flowData, deployed: true },
        { headers }
      );
